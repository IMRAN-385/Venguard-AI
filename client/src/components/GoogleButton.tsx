...
export function GoogleButton({ label, onSuccess }: Props) {
  ...
  async function handleGoogle() {
    setLoading(true);
    try {
      // In production: use @react-oauth/google or NextAuth to get id_token
      // For now, POST to backend which returns a demo Google session
      const { data } = await api.post("/auth/google-login", {
        token: "demo-google-id-token",
      });
      if (data.token) {
      ...
      alert("Google sign-in unavailable — backend offline.");
    } finally {
      setLoading(false);
    }
  }
